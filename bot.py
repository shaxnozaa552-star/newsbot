...
def create_flask_app():
    """Создает и запускает Flask сервер в фоне."""
    app = Flask('')
...
class NewsBot:
    def __init__(self):
        ...
        self.CATEGORIES = {
            "политика": ["https://lenta.ru/rss", "https://ria.ru/export/rss2/politics.xml", "https://www.vedomosti.ru/rss/news"],
            "экономика": ["https://www.vedomosti.ru/rss/news", "https://ria.ru/export/rss2/economy.xml", "https://www.kommersant.ru/RSS/news.xml"],
            "технологии": ["https://habr.com/ru/rss/hub/python/", "https://vc.ru/rss", "https://3dnews.ru/news/rss/"],
            "спорт": ["https://www.championat.com/rss/news.xml", "https://www.sports.ru/rss/rubric.xml?s=208", "https://rsport.ria.ru/export/rss2/index.xml"],
            "культура": ["https://www.kp.ru/rss/theme/10/", "https://www.kommersant.ru/RSS/section-culture.xml", "https://rg.ru/rss/culture.xml"],
        }

    # ========== КЛАВИАТУРЫ ==========
    def get_start_menu(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Начать работу", callback_data="start_work")]])

    def get_categories_keyboard(self, selected: Set[str] = None) -> InlineKeyboardMarkup:
        selected = selected or set()
        keyboard = [[InlineKeyboardButton(f"{'✅' if category in selected else '⚪'} {category}", callback_data=f"cat_{category}")] for category in self.CATEGORIES]
        ...
        try:
            ...
            for entry in feed.entries[:8]:
                ...
                articles.append(NewsArticle(
                    id=news_id, title=title, link=entry.link,
                    category=category, source=feed_url.split('/')[2], timestamp=time.time()
                ))
            return articles
    ...
    def fetch_fresh_news(self, categories: List[str]) -> str:
        if not categories: return "❌ Выберите категории в настройках"
        all_articles = []
    ...
    async def _handle_start(self, query, user_id):
        await query.edit_message_text("📋 Выберите категории новостей:", reply_markup=self.get_categories_keyboard())

    async def _handle_category_select(self, query, user_id, data):
        category = data.replace("cat_", "")
        if user_id not in self.subscribers: self.subscribers[user_id] = set()
        user_cats = self.subscribers[user_id]
        if category in user_cats: user_cats.remove(category)
        else: user_cats.add(category)
        await query.edit_message_reply_markup(reply_markup=self.get_categories_keyboard(user_cats))
    ...
    async def _handle_save_cats(self, query, user_id):
        if user_id in self.subscribers and self.subscribers[user_id]:
            ...
            await query.edit_message_text(f"✅ Подписка оформлена!\n\n📋 Категории: {', '.join(cats)}\n\n⏰ Рассылка в 09:00 и 18:00\n\n📱 Главное меню:", reply_markup=self.get_main_menu())
        else:
            await query.edit_message_text("❌ Выберите хотя бы одну категорию!", reply_markup=self.get_categories_keyboard())
    ...
    async def _handle_get_news(self, query, user_id):
        ...
        await query.message.reply_text("🔄 Нажмите 'Получить новости' для следующих новостей!\n\n📱 Меню:", reply_markup=self.get_main_menu())

    async def _handle_change_cats(self, query, user_id):
        current_cats = self.subscribers.get(user_id, set())
        await query.edit_message_text("📋 Изменить категории:", reply_markup=self.get_categories_keyboard(current_cats))

    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text("👋 Добро пожаловать в NewsBot!\n\n📰 Ваш персональный агрегатор новостей.\n🔄 Каждый раз новые свежие новости!", reply_markup=self.get_start_menu())
    ...
    async def news(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🔄 Используйте /news для следующих новостей!\n\n📱 Меню:", reply_markup=self.get_main_menu())
    ...
    async def send_news_to_all(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        for user_id, categories in self.subscribers.items():
            ...
            try:
                await context.bot.send_message(chat_id=user_id, text=news_text, parse_mode="HTML")
            except Exception as e:
...
def main():
    ...
    if application.job_queue:
        application.job_queue.run_daily(bot.send_news_to_all, time=dtime(hour=9, minute=0), name="morning_news")
        application.job_queue.run_daily(bot.send_news_to_all, time=dtime(hour=18, minute=0), name="evening_news")
